        ...
        <table class="table table-striped table-bordered shadow-lg mt-4 table-responsive-xl" style="width:100%" id="tb">
            <thead class="bg-success text-white">
                <tr>
                    ...
                    <th scope="col">
                        <a href="#proveedor" data-toggle="modal" title="Crear Proveedor" class="btn btn-warning">Nuevo</a>
                    </th>
                </tr>
            </thead>
            <tbody>
                @foreach ($pv as $item)
                <?php $stts = ($item->estado_pv == 'ACTIVO') ? 'bg-gradient-success text-white' : 'bg-gradient-danger text-white'; ?>
                <tr>
                    <td scope="row">{{$item->dni_pv}}</td>
                    <td scope="row">{{$item->nombre_pv}}</td>
                    <td scope="row">{{$item->contacto_pv}}</td>
                    <td scope="row">{{$item->direccion_pv}}</td>
                    <td scope="row">{{$item->telefono_pv}}</td>
                    <td scope="row"><span class="btn btn-sm rounded block <?php echo $stts ?>" style="opacity: 0.75; width: 100%;">{{$item->estado_pv}}</span></td>
                    <td scope=" row">
                        <a href="/proveedores/{{$item->idProveedor}}/edit" class="btn btn-primary"><i class="fas fa-edit"></i></a>
                        <a href="#" class="btn btn-danger" onclick="confirmarEliminacion(event, '{{$item->idProveedor}}')"><i class="fas fa-trash-alt"></i></a>
                    </td>
                </tr>
                @endforeach
            </tbody>
...
<div class="modal fade" tabindex="-1" id="proveedor">
            ...
            <div class="modal-body">
                <div class="card">
                    <div class="card-body">
                        <p>
                            <center>
                                <form action="/proveedores" method="post" id="myForm">
                                    {{ csrf_field() }}
                                    <div class="row g-3">
                                        <div class="col-md-12">
                                            <label for="dni" class="form-label">No. Identificación/NIT</label>
                                            <input type="number" class="form-control" name="dni_pv" id="dni" required placeholder="Número Identificacion">
                                            <span id="dni-error" class="text-danger"></span>
                                        </div>
                                        <div class="col-md-6">
                                            <label for="name" class="form-label">Nombre/Razon Social</label>
                                            <input type="text" class="form-control" name="nombre_pv" id="name" required placeholder="Razon social">
                                            <span id="name-error" class="text-danger"></span>
                                        </div>
                                        <div class="col-md-6">
                                            <label for="contact" class="form-label">Nombre Contacto</label>
                                            <input type="text" class="form-control" name="contact" id="contact" placeholder="Julio Buelvas">
...
<script>
    document.getElementById('dni').addEventListener('blur', function() {
        var dni = this.value;
        if (dni) {
            axios.get('{{ route("proveedores.validar_dni") }}', {
                    params: {
                        dni: dni
                    }
                })
                .then(function(response) {
                    ...
                    if (response.data.exists) {
                        dniError.textContent = 'Error, proveedor ya está registrado';
                        AlertSweet("Proveedor ya está registrado!", "Favor verificar la información.", "error");
    ...
    document.getElementById('btnEnvio').addEventListener('click', function() {
        ...
        const cont = document.getElementById('contact').value;
        ...
        if (dni === '' || dni.length === 0) {
            ...
            if (nom === '' || nom.length === 0) {
                ...
                /* Swal.fire({
                    title: "Información Enviada Correctamente!",
                    text: "Click para finalizar el proceso de registro!",
                    icon: "success"
                }).then((result) => {
                    if (result.isConfirmed) {
                        document.getElementById('myForm').submit();
